...
export class TableService {
    ...
    /**
     * The functions is used to add new records in existingData, 
     *  1. add new records if any
     *  2. delete the records if any 
     *  3. sorts the final array according to sortedIds
     * @param newData the Data returned from API - holds the updated/new objects
     * @param existingData the existing data in the page
     * @param sortedIds the final sorted array of record ids
     */
    setPageData(newData: any[], existingData: any[], sortedIds: any[], key: string) {
        let finalRecords = [];
        existingData.forEach(f=> {
            let existInFinal = sortedIds.findIndex(s=> f[key] === s);
            if (existInFinal > -1) {
                f.sequence = existInFinal;
                finalRecords.push(f)
            }
        })
        newData.forEach(f=> {
            let isNewRecord = finalRecords.findIndex(r=> r[key] === f[key]);
            if (isNewRecord < 0) {
                let finalIndex = sortedIds.findIndex(s=> f[key] === s);
                f.sequence = finalIndex;
                finalRecords.push(f);
            } else {
                finalRecords[isNewRecord] = f
            }
        })
        finalRecords.sort((a,b)=> a.sequence - b.sequence);
        return finalRecords;
    }
}
